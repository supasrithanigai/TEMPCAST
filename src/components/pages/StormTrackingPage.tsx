...
export const StormTrackingPage: React.FC<StormTrackingPageProps> = ({
  ...
  const currentStorm = selectedStorm || (storms.length > 0 ? storms[0] : null);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {storms.length === 0 ? (
        <div className="rounded-xl bg-slate-900/90 border border-slate-800 p-8 text-center space-y-4">
          <div className="w-12 h-12 rounded-xl bg-slate-800 text-amber-400 flex items-center justify-center mx-auto">
            <FileQuestion className="w-6 h-6" />
          </div>
          <div className="space-y-1">
            <h3 className="text-base font-bold text-white">Awaiting Original Dataset</h3>
            <p className="text-xs text-slate-400 max-w-md mx-auto">
              No storm cells are currently detected in uploaded radar datasets. Upload Doppler radar volume files or satellite datasets in the <strong>Datasets</strong> section to track storm centroids.
            </p>
          </div>
        </div>
      ) : currentStorm ? (
